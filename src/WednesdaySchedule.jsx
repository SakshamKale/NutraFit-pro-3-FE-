...
let nav = useNavigate();
  useEffect(() => {
    const localData = JSON.parse(localStorage.getItem('selectedData3'));
    ...
    <>
      ...
      <div className="box21">
        <div className="inbox21">
          ...
          <span style={{ backgroundColor: '#1F2B3F', color: 'white', borderRadius: '5px', width: '50px', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>WED</span>
      ...
      <button className="plan-btn" onClick={()=>{nav('/wednesdaymenu')}}>
  + Plan your Day
</button>
    </>
  );
