...
const MonMenu = () => {
  const mealMap = ['bf', 'bf', 'lun', 'lun', 'din'];
  ...
  const handleSelect = (sectionIndex, item) => {
    ...
    updated[sectionIndex] = { src: item.src, text: item.text, meal: mealMap[sectionIndex] };
    setSelected(updated);
  };
  ...
  const handleAddCustomItem = (sectionIndex) => {
    ...
    const newItem = { src: '', text: inputValue }; // No image for custom items
    // Add custom item to menuSections
    ...
    updatedSelected[sectionIndex] = { ...newItem, meal: mealMap[sectionIndex] };
  ...
  const handleDone = () => {
    localStorage.setItem('selectedData1', JSON.stringify(selected));
        ...
        <div className="box30" key={section.title}>
          ...
          <div className="inbox30">
            {section.items.map((item, itemIndex) => (
              ...
              >
                <img
                  src={item.src || 'https://via.placeholder.com/80?text=No+Image'}
                  alt=""
                  className="img5"
                  style={
          ...
          <div className="input-arrow-wrapper" style={{ marginTop: '8px' }}>
  <input
    type="text"
