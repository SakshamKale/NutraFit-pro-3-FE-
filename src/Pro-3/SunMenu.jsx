...
const SunMenu = () => {
  ...
  const handleDone = () => {
    localStorage.setItem('selectedData7', JSON.stringify(selected));
    window.location.href = '/sunday';
  };
useEffect(() => {
  const saved = JSON.parse(localStorage.getItem('selectedData7'));
        ...
        <div className="box30" key={section.title}>
          ...
          >
            <input
              ...
              onChange={(e) =>
                handleCustomInputChange(sectionIndex, e.target.value)
              }
              onKeyDown={(e) => {
